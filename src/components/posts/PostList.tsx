              ...
              >
                <div className="flex gap-4">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-gray-600 mb-2" dangerouslySetInnerHTML={{ __html: DOMPurify.sanitize(truncateHtml(post.content, 150)) }} />
                    <div className="flex flex-wrap gap-2 mt-4">
                      {post.tags?.map((tag) => (
                        <Badge
                          key={tag.id}
                          className={`${getTagColor(tag.name)} cursor-default`}
                          variant="secondary"
                        >
                          {tag.name}
                        </Badge>
                ...
                <AlertDialog>
                  ...
                  <AlertDialogContent onClick={(e: any) => e.stopPropagation()}>
                    ...
                    <AlertDialogFooter>
                      ...
                      <AlertDialogAction
                        onClick={() => handleDeletePost(post.id ?? 0)}
                      >
                        Eliminar
                      </AlertDialogAction>
                    </AlertDialogFooter>
          ...
          <DialogContent className="sm:max-w-2xl max-h-[90vh] p-0">
            <ScrollArea className="h-full max-h-[90vh]">
              <div className='p-6'>
                ...
                <div className="mt-4">
                  <p className="text-gray-600" dangerouslySetInnerHTML={{ __html: DOMPurify.sanitize(selectedPost.content) }} />
                </div>
          ...
          <ScrollArea className="max-h-[500px] overflow-y-auto px-0 py-0">
            <div className="grid gap-4 py-4 px-4">
              ...
              <div className="space-y-2">
                ...
                <TagSelector2
                  selectedTags={selectedTagsToCreate}
                  onTagsChange={setSelectedTagsToCreate}
                />
              </div>
            </div>
          </ScrollArea>
...
const ImageOrFallback = ({ src, alt, className, isHeader = false }: {
  src?: string;
  alt: string;
  className?: string;
  isHeader?: boolean;
  ...
  if (!src || hasError) {
    return (
      <div
        className={`${className} ${colorClass} ${containerClass} flex items-center justify-center`}
        aria-label={alt}
      >
